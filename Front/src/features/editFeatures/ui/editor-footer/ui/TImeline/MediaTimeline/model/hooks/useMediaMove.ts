...
export function useMediaMove() {
  ...
  const choosePrimaryOverlap = useCallback(
    (targetStartTime: number, duration: number, excludeId: string) => {
      const overlaps = findOverlappingElements(targetStartTime, duration, excludeId);
      if (overlaps.length === 0) return null;

      const draggedCenter = roundTime(targetStartTime + duration / 2);
      let best = overlaps[0];
      let bestDelta = Math.abs(draggedCenter - roundTime((overlaps[0].startTime + overlaps[0].endTime) / 2));

      for (let i = 1; i < overlaps.length; i += 1) {
        const candidate = overlaps[i];
        const candidateCenter = roundTime((candidate.startTime + candidate.endTime) / 2);
  ...
  const hasOverlapAt = useCallback(
    (candidateStart: number, duration: number, excludeId: string) => {
      ...
      for (const element of getSortedElements(excludeId)) {
        const elementStart = roundTime(element.startTime);
        const elementEnd = roundTime(element.endTime);
        if (candidateStartRounded < elementEnd && candidateEndRounded > elementStart) {
          return true;
        }
      }
      return false;
  ...
  const computeSnapStartForPreview = useCallback(
    (targetStartTime: number, duration: number, excludeId: string): number => {
      const primary = choosePrimaryOverlap(targetStartTime, duration, excludeId);
      if (!primary) {
        return calculateValidDropTime(targetStartTime, duration, excludeId);
      }

      const primaryCenter = roundTime((primary.startTime + primary.endTime) / 2);
      const draggedCenter = roundTime(targetStartTime + duration / 2);

      let candidateStart: number;
      if (draggedCenter < primaryCenter) {
        // Place before overlapping element so dragged end touches its start
        candidateStart = roundTime(Math.max(0, roundTime(primary.startTime) - duration));
      } else {
      ...
      if (hasOverlapAt(candidateStart, duration, excludeId)) {
        ...
        if (draggedCenter < primaryCenter) {
          ...
          alternative = roundTime(Math.max(0, roundTime(primary.startTime) - duration));
        }
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      const validTargetTime = computeSnapStartForPreview(targetTime, duration, moveDragState.elementId);
    ...
    [moveDragState, pixelsToTime, timeToPixels, computeSnapStartForPreview, roundTime]
  );
  ...
  const handleMouseUp = useCallback(() => {
    if (moveDragState.isDragging && moveDragState.elementId && dropPreview.isVisible) {
      ...
      const resolvedStartTime = computeSnapStartForPreview(rawStartTime, duration, moveDragState.elementId);
  ...
  }, [moveDragState, dropPreview, updateMediaElement, roundTime, computeSnapStartForPreview]);
