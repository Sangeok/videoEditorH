...
import { ResizeDragState, ResizeDragType, ElementAdjustment } from "../types";
...
interface UseTrackElementDragProps<
...
export function useTrackElementDrag<
  T extends MediaElement | AudioElement | TextElement
>({
  SelectedElements,
  updateSelectedElements,
  updateMultipleSelectedElements,
}: UseTrackElementDragProps<T>) {
  ...
  const handleRightResize = useCallback(
    (deltaTime: number) => {
      ...
      let candidateEndTime = Math.max(
        dragState.originalStartTime + 0.1,
        dragState.originalEndTime + deltaTime
      );
      candidateEndTime = roundTime(candidateEndTime);

      const isExtendingBeyondMax =
        candidateEndTime >
